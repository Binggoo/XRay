...
* Version            : V1.0.1
* Date               : 06/13/2008
* Description        : Ini class implementation
...
#pragma warning(push)
#endif
#pragma warning(disable:4244) // named type definition in parentheses
#pragma warning(disable:4311) // named type definition in parentheses
#pragma warning(disable:4267) // named type definition in parentheses
#pragma warning(disable:4995) // named type definition in parentheses
#pragma warning(disable:4996) // named type definition in parentheses
...
__int64 CIni::GetInt64(LPCTSTR lpSection, LPCTSTR lpKey, __int64 nDefault, int nBase /* = BASE_DECIMAL */) const
{
	TCHAR sz[DEF_PROFILE_NUM_LEN + 1] = _T("");
	GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
	return *sz == _T('\0') ? nDefault : _tcstoi64(sz, NULL, __ValidateBase(nBase));
}
...
UINT64 CIni::GetUInt64(LPCTSTR lpSection, LPCTSTR lpKey, UINT64 nDefault, int nBase /* = BASE_DECIMAL */) const
{
	TCHAR sz[DEF_PROFILE_NUM_LEN + 1] = _T("");
	GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
	return *sz == _T('\0') ? nDefault : UINT64(_tcstoui64(sz, NULL, __ValidateBase(nBase)));
}
...
BOOL CIni::WriteInt64(LPCTSTR lpSection, LPCTSTR lpKey, __int64 nValue, int nBase /* = BASE_DECIMAL */) const
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
...
BOOL CIni::WriteUInt64(LPCTSTR lpSection, LPCTSTR lpKey, UINT64 nValue, int nBase /* = BASE_DECIMAL */) const
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
	__UInt64ToString(nValue, szValue, nBase);
	return WriteString(lpSection, lpKey, szValue);
}
...
BOOL CIni::IncreaseInt64(LPCTSTR lpSection, LPCTSTR lpKey, __int64 nIncrease /* = 1 */, int nBase /* = BASE_DECIMAL */) const
{
	__int64 nVal = GetInt64(lpSection,lpKey,0,nBase);
	nVal += nIncrease;

	return WriteInt64(lpSection,lpKey,nVal,nBase);
}
...
BOOL CIni::IncreaseUInt64(LPCTSTR lpSection, LPCTSTR lpKey, UINT64 nIncrease /* = 1 */, int nBase /* = BASE_DECIMAL */) const
{
	UINT64 nVal = GetUInt64(lpSection, lpKey, 0, nBase);
	nVal += nIncrease;
	return WriteUInt64(lpSection, lpKey, nVal, nBase);
}
...
void CIni::__Int64ToString(__int64 nNumber,LPTSTR lpBuffer, int nBase)
{
	...
	case BASE_OCTAL:
	case BASE_HEXADECIMAL:
		__UInt64ToString((UINT64)nNumber, lpBuffer, nBase);
		break;

	default:
		_stprintf(lpBuffer, _T("%I64d"), nNumber);
		break;
	}	
}
...
void CIni::__UInt64ToString(UINT64 nNumber, LPTSTR lpBuffer, int nBase)
{
	switch (nBase)
	{
	case BASE_BINARY:
	...
	case BASE_OCTAL:
		_stprintf(lpBuffer, _T("%I64o"), nNumber);
		break;

	case BASE_HEXADECIMAL:
		_stprintf(lpBuffer, _T("%I64X"), nNumber);
		break;

	default:
		_stprintf(lpBuffer, _T("%I64u"), nNumber);
		break;
	}	
}
