...
void CEditList::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
		...
		plvItem->pszText )		// valid text
	{
		...
		{
			CString strText;
			strText = GetItemText( plvItem->iItem, 0);				
			SetItemText( plvItem->iItem, 0, strText);				
		}	
	}
...
void CEditList::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (m_bEditable)
	{
		if ( m_fGetType )
		{
			switch (m_fGetType( pNMListView->iSubItem ))
			{
			case eCombo:
	...
	::SendMessage(GetParent()->GetSafeHwnd(),WM_USER_CLICK,(WPARAM)pNMHDR,(LPARAM)pResult);
